...
reference_data = repair.repair_with_reference(unionized_data, datasets, datasets_composition, data_context_columns, target_schema, repaired_columns)

fds_data = repair.repair_with_fds(unionized_data, fd_list, cfds)
...
# join data with additional datasets

inner_reference_data = mapping.join_additional_data(reference_data, datasets, datasets_composition, add_data_columns, 'inner', 'postcode')
inner_reference_data = inner_reference_data[['provenance', 'postcode', 'price', 'street_name', 'bedroom_number', 'crimerank']]

outer_reference_data = mapping.join_additional_data(reference_data, datasets, datasets_composition, add_data_columns, 'left', 'postcode')
outer_reference_data = outer_reference_data[['provenance', 'postcode', 'price', 'street_name', 'bedroom_number', 'crimerank']]

inner_fds_data = mapping.join_additional_data(fds_data, datasets, datasets_composition, add_data_columns, 'inner', 'postcode')
iinner_fds_data = inner_fds_data[['provenance', 'postcode', 'price', 'street_name', 'bedroom_number', 'crimerank']]

outer_fds_data = mapping.join_additional_data(fds_data, datasets, datasets_composition, add_data_columns, 'left', 'postcode')
...
inner_reference_data.to_csv('data/results/repair_after_union_inner_reference.csv', sep=',', encoding='utf-8', index=False)
outer_reference_data.to_csv('data/results/repair_after_union_outer_reference.csv', sep=',', encoding='utf-8', index=False)
inner_fds_data.to_csv('data/results/repair_after_union_inner_fds.csv', sep=',', encoding='utf-8', index=False)
outer_fds_data.to_csv('data/results/repair_after_union_outer_fds.csv', sep=',', encoding='utf-8', index=False)
